...
class _MatchingCompleteScreenState extends State<MatchingCompleteScreen>
  ...
  int _computeTimeScore() {
    final targetSeconds = max(20, (widget.pairCount ?? 6) * 9);
    final elapsed = max(1, widget.elapsedSeconds);
    final raw = (targetSeconds / elapsed) * 100;
    return raw.clamp(35, 100).round();
  }

  int _computeFinalScore() {
    final timeScore = _computeTimeScore();
    return ((widget.accuracy * 0.75) + (timeScore * 0.25)).round().clamp(
      0,
      100,
    );
  }
