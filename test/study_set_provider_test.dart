...
void main() {
  ...
  setUpAll(() async {
    tempDir = await Directory.systemTemp.createTemp('Recall-study-set-provider-');
  ...
  test('moveToFolder updates folderId and marks set unsynced', () async {
    final original = StudySet(
      id: 'set_1',
      title: 'Set 1',
    ...
    await localStorage.saveStudySet(original);

    final container = buildContainer();
    addTearDown(container.dispose);

    await container.read(studySetsProvider.notifier).moveToFolder(
          'set_1',
          'folder_1',
        );

    final updated = localStorage.getStudySet('set_1');
    expect(updated, isNotNull);
    expect(updated!.folderId, 'folder_1');
